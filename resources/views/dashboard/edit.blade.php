...
<form action="/dashboard/blogs/{{ $data->id }}" method="POST" enctype="multipart/form-data" class="space-y-6 max-w-lg">
    @method('put')
    @csrf
    ...
    <div>
        ...
        <input type="text" name="judul" value="{{ old('judul',$data->judul) }}" class="w-full border border-gray-300 rounded px-4 py-2 focus:outline-none focus:ring-2 focus:ring-blue-400">
        @error('judul')
    ...
    <div>
        ...
        @if($data->gambar)
        <img class="img-fluid img-preview" src="{{ asset('storage/' . $data->gambar) }}"  alt="">
        @else
    ...
    <div>
        ...
        <textarea name="isi" id="isi" cols="30" rows="10" class="w-full border border-gray-300 rounded px-4 py-2 focus:outline-none focus:ring-2 focus:ring-blue-400">{{ old('isi',$data->isi) }}</textarea>
        @error('isi')
            <p class="text-red-500 mt-1 text-sm">{{ $message }}</p>
        @enderror
    </div>
